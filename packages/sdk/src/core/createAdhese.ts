...
/**
 * Platform-agnostic core factory for creating an Adhese instance.
 * Platform-specific behavior (DOM rendering, consent APIs, device detection, etc.)
 * is injected via the `platform` parameter.
 *
 * @param options {AdheseOptions} The options to create the Adhese instance with.
 * @param platform {PlatformAdapters} Platform-specific adapters for slot creation, consent, etc.
 *
 * @return Adhese The Adhese instance.
 */
export function createAdheseCore<T extends ReadonlyArray<AdhesePlugin>>(
  ...
  return scope.run(() => {
    ...
    // Platform-specific parameter initialization
    platform.useParameters?.(context, mergedOptions);

    const slotManager = createSlotManager({
      initialSlots: mergedOptions.initialSlots,
      context,
      createSlot: platform.createSlot,
      findDomSlots: platform.findDomSlots,
    });
    ...
    platform.useDebugMode?.(context);

    // Platform-specific device detection
    platform.useDeviceDetection?.(context, mergedOptions);

    // Platform-specific consent handling
    platform.useConsent?.(context);
